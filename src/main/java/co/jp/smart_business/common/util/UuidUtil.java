...
/**
 * UuidUtil designed for Generating short hashes from numbers (like YouTube and Bitly), obfuscate
 * database IDs, use them as forgotten password hashes, invitation codes, store shard numbers.
 * <p>
 * This is implementation of http://UuidUtil.org v1.0.0 version.
 *
...
public class UuidUtil {
  /**
   * Max number that can be encoded with UuidUtil.
   */
  ...
  public UuidUtil(String salt) {
    this(salt, 0);
  }

  public UuidUtil(String salt, int minHashLength) {
    this(salt, minHashLength, DEFAULT_ALPHABET);
  }

  public UuidUtil(String salt, int minHashLength, String alphabet) {
    ...
    seps = UuidUtil.consistentShuffle(seps, this.salt);
    ...
    alphabet = UuidUtil.consistentShuffle(alphabet, this.salt);
  ...
  private String _encode(long... numbers) {
    ...
    for (int i = 0; i < numbers.length; i++) {
      ...
      alphabet = UuidUtil.consistentShuffle(alphabet, buffer.substring(0, alphabet.length()));
      final String last = UuidUtil.hash(num, alphabet);
    ...
    while (ret_str.length() < this.minHashLength) {
      alphabet = UuidUtil.consistentShuffle(alphabet, alphabet);
    ...
    if (hashArray.length > 0) {
      ...
      if (!hashBreakdown.isEmpty()) {
        ...
        for (final String aHashArray : hashArray) {
          ...
          alphabet = UuidUtil.consistentShuffle(alphabet, buffer.substring(0, alphabet.length()));
          ret.add(UuidUtil.unhash(subHash, alphabet));
        }
      }
    }
   ...
   *
   * @return UuidUtil algorithm version implemented.
   */
